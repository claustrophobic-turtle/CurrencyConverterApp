...
class MainActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val btnRupee: Button = findViewById(R.id.btn_rupee)
        val btnDollar: Button = findViewById(R.id.btn_dollar)
        val convertedText: TextView = findViewById(R.id.text_amount)

        btnRupee.setOnClickListener {
            val amount = edt.text.toString().toInt()
            val rupeeAmount = 69 * amount
            convertedText.text = "Converted Amount: $rupeeAmount"
        }

        btnDollar.setOnClickListener {
            val amount = edt.text.toString().toInt()
            val dollarAmount = amount / 69
